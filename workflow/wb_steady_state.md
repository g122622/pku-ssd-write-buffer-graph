...
你需要执行`/mnt/wsl-share/fio_scripts`下的`wb_steady_state.sh`。脚本会自动跑并写入结果到指定文件（可能不止产生一个文件）。(注意：脚本由wsl host共享，在wsl中位于`/mnt/d/MiscProjects/pku-ssd-write-buffer-graph/fio_scripts`下)
